...
*             Add if statement to remove the error of 'draw rec' 
*             y must be bigger than y0 (1.5)
*  2008-12-01 Yucheng Song changed this to plot stations histogram
*  2010-11-29 Yucheng Song changed to bigger graph 
function main(args)
...
title='Expected forecast error reduction in verification region (VR) due to adaptive observations by rawinsondes'
'set string 1 bc 6'
'set strsiz 'hsiz' 'vsiz
'draw string 'xstart' 'ystart' '%title

*title2='Case 'case'  Obs. time: 'obsdate'  Verif. time 'veridate'  VR: 'vrlat'N, 'vrlon', 'radvr'km radius  Verif. var.: 'vnormgr' '
title2='VR: 'vrlat'N, 'vrlon', Verif. time 'veridate' Obs. time: 'obsdate' (VR radius: 'radvr'km)  Verif. var.: 'vnormgr' '
ystart = ystart - 0.25
'set string 1 bc 6'
'set strsiz 'hsiz' 'vsiz
...
xstart = xstart - 0.5
ystart = ystart - 0.25
title3='NCEP/UM ETKF based on 'mem'-member 'ensdate' 'ensemble' ensemble.'
'set string 1 bc 6'
'set strsiz 'hsiz' 'vsiz
...
*title2='Verification region at 'vrlon' 'vrlat'N, radius 'radvr'km.'
